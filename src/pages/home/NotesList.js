...
const NotesList = ({ filterText, notes, handleSetActiveNote, isLoading, error, activeNote }) => {
  ...
  const notesElements = nonEmptyNotesList
    ? notes.map(note => <NoteItem key={note.id} value={note} activeNote={activeNote} handleSetActiveNote={handleSetActiveNote} />)
    : null;
